...
public class PolygonFollower extends SubFollower {
    ...
    private void computeOriginalPoints() {
        ...
        originalPoints = new ArrayList<>(
                Arrays.asList(PolygonUtils.mergeTouchingPolygonsToOne(polygonComponent.vertices)));
    ...
    public void drawOutlines() {
        if (drawPoints.length > 0) {
            ...
            for (int i = 1; i < drawPoints.length; i++) {
                ...
                shapeRenderer.line(drawPoints[i].x * transformComponent.scaleX,
                        drawPoints[i].y * transformComponent.scaleY, drawPoints[i - 1].x * transformComponent.scaleX,
                        drawPoints[i - 1].y * transformComponent.scaleY);
            }
            shapeRenderer.setColor(outlineColor);
            ...
            shapeRenderer.line(drawPoints[drawPoints.length - 1].x * transformComponent.scaleX,
                    drawPoints[drawPoints.length - 1].y * transformComponent.scaleY,
                    drawPoints[0].x * transformComponent.scaleX, drawPoints[0].y * transformComponent.scaleY);
            shapeRenderer.end();
        }
    ...
    public void drawTriangulatedPolygons() {
        ...
        for (Vector2[] poly : polygonComponent.vertices) {
            for (int i = 1; i < poly.length; i++) {
                shapeRenderer.line(poly[i - 1].x * transformComponent.scaleX, poly[i - 1].y * transformComponent.scaleY,
                        poly[i].x * transformComponent.scaleX, poly[i].y * transformComponent.scaleY);
            }
            if (poly.length > 0)
                shapeRenderer.line(poly[poly.length - 1].x * transformComponent.scaleX,
                        poly[poly.length - 1].y * transformComponent.scaleY, poly[0].x * transformComponent.scaleX,
                        poly[0].y * transformComponent.scaleY);
        }
        shapeRenderer.end();
    }
        ...
        addListener(new ClickListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (anchorId >= 0) {
                    ...
                    listener.anchorDown(PolygonFollower.this, anchorId,
                            x * runtimeCamera.zoom / transformComponent.scaleX,
                            y * runtimeCamera.zoom / transformComponent.scaleY);
                } else if (lineIndex > -1) {
                    // not anchor but line is selected gotta make new point
                    listener.vertexDown(PolygonFollower.this, lineIndex,
                            x * runtimeCamera.zoom / transformComponent.scaleX,
                            y * runtimeCamera.zoom / transformComponent.scaleY);
                }
                return true;
            ...
            public void touchDragged(InputEvent event, float x, float y, int pointer) {
                ...
                if (anchorId >= 0) {
                    listener.anchorDragged(PolygonFollower.this, anchorId,
                            x * runtimeCamera.zoom / transformComponent.scaleX,
                            y * runtimeCamera.zoom / transformComponent.scaleY);
                } else if (lineIndex > -1) {
            ...
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (anchorId >= 0) {
                    listener.anchorUp(PolygonFollower.this, anchorId,
                            x * runtimeCamera.zoom / transformComponent.scaleX,
                            y * runtimeCamera.zoom / transformComponent.scaleY);
                } else if (lineIndex > -1) {
                    listener.vertexUp(PolygonFollower.this, lineIndex,
                            x * runtimeCamera.zoom / transformComponent.scaleX,
                            y * runtimeCamera.zoom / transformComponent.scaleY);
                }
                draggingAnchorId = -1;
    ...
    private int vertexHitTest(float x, float y) {
        ...
        for (int i = 1; i < drawPoints.length; i++) {
            Vector2 pointOne = drawPoints[i - 1].cpy()
                                                .scl(1f / runtimeCamera.zoom * transformComponent.scaleX,
                                                        1f / runtimeCamera.zoom * transformComponent.scaleY);
            Vector2 pointTwo = drawPoints[i].cpy()
                                            .scl(1f / runtimeCamera.zoom * transformComponent.scaleX,
                                                    1f / runtimeCamera.zoom * transformComponent.scaleY);
        ...
        Vector2 pointOne = drawPoints[drawPoints.length - 1].cpy()
                                                            .scl(1f / runtimeCamera.zoom * transformComponent.scaleX,
                                                                    1f / runtimeCamera.zoom * transformComponent
                                                                            .scaleY);
        Vector2 pointTwo = drawPoints[0].cpy()
                                        .scl(1f / runtimeCamera.zoom * transformComponent.scaleX,
                                                1f / runtimeCamera.zoom * transformComponent.scaleY);
    ...
    private int anchorHitTest(float x, float y) {
        ...
        for (int i = 0; i < drawPoints.length; i++) {
            Circle pointCircle = new Circle(drawPoints[i].x / runtimeCamera.zoom * transformComponent.scaleX,
                    drawPoints[i].y / runtimeCamera.zoom * transformComponent.scaleY,
                    (float) CIRCLE_RADIUS / pixelsPerWU);
