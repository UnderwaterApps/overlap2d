...
public class SandboxInputAdapter implements InputProcessor {
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (screenX < viewPort.getScreenX() || screenX >= viewPort.getScreenX() + viewPort.getScreenWidth())
            return false;
        if (Gdx.graphics.getHeight() - screenY < viewPort.getScreenY() || Gdx.graphics.getHeight() - screenY >= viewPort
                .getScreenY() + viewPort.getScreenHeight())
            return false;
        ...
        if (target == null) {
            ...
            Array<InputListener> sandboxListeners = sandbox.getAllListeners();
            for (int i = 0, s = sandboxListeners.size; i < s; i++) {
                sandboxListeners.get(i)
                                .touchDown(null, hitTargetLocalCoordinates.x, hitTargetLocalCoordinates.y, pointer,
                                        button);
            }
        ...
        if (inpputListenerComponent == null)
            return false;
        ...
        for (int j = 0, s = listeners.size; j < s; j++) {
            if (listeners.get(j)
                         .touchDown(target, hitTargetLocalCoordinates.x, hitTargetLocalCoordinates.y, pointer,
                                 button)) {
                return true;
            }
        }
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        ...
        if (target == null) {
            ...
            for (int i = 0, s = sandboxListeners.size; i < s; i++) {
                sandboxListeners.get(i)
                                .touchUp(null, hitTargetLocalCoordinates.x, hitTargetLocalCoordinates.y, pointer,
                                        button);
            }
    ...
    public boolean touchDragged(int screenX, int screenY, int pointer) {
        ...
        if (target == null) {
            ...
            for (int i = 0, s = sandboxListeners.size; i < s; i++) {
                sandboxListeners.get(i)
                                .touchDragged(null, hitTargetLocalCoordinates.x, hitTargetLocalCoordinates.y, pointer);
            }
            return false;
        }
